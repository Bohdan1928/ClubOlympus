...
public class AddMemberActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private static final int EDIT_LOADER = 111;
    ...
    private void saveMember() {
        ...
        if (TextUtils.isEmpty(firstName)) {
            Toast.makeText(this,
                    "Input the name", Toast.LENGTH_LONG).show();
            return;
        } else if (TextUtils.isEmpty(lastName)) {
            Toast.makeText(this,
                    "Input the last name", Toast.LENGTH_LONG).show();
            return;
        } else if (gender == MemberEntry.GENDER_UNKNOWN) {
            Toast.makeText(this,
                    "Choose the gender", Toast.LENGTH_LONG).show();
            return;
        } else if (TextUtils.isEmpty(sport)) {
            Toast.makeText(this,
                    "Input the sport", Toast.LENGTH_LONG).show();
            return;
        }
        ContentValues contentValues = new ContentValues();
        contentValues.put(MemberEntry.KEY_NAME, firstName);
        ...
        if (currentMemberUri == null) {
            ...
            if (uri == null) {
                Toast.makeText(this, "Inserting of data the table failed", Toast.LENGTH_LONG).show();
            } else {
    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor data) {
        if (data.moveToFirst()) {
            int firstNameColumnIndex = data.getColumnIndex(MemberEntry.KEY_NAME);
            int lastNameColumnIndex = data.getColumnIndex(MemberEntry.KEY_LASTNAME);
    ...
    private void deleteMember() {
        if (currentMemberUri != null) {
            int rowsDeleted = getContentResolver().delete(currentMemberUri,
                    null, null);
            if (rowsDeleted == 0) {
                Toast.makeText(this, "deleting of data from the table failed", Toast.LENGTH_SHORT).show();
            } else {
